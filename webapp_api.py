...
def _write_or_403(user_id: int) -> web.Response | None:
...
async def update_transaction(request: web.Request) -> web.Response:
    ...
    if error is not None:
        return error
    ok = db.update_transaction_fields(
        user_id,
        tx_id,
        tx_type=parsed["type"],
        amount=parsed["amount"],
...
def _serialize_budget(user_id: int, b) -> dict:
...
async def get_budgets_status(request: web.Request) -> web.Response:
    ...
    if not user_id:
        return web.json_response({"error": "unauthorized"}, status=401)
    items = [_serialize_budget(user_id, b) for b in db.get_budgets(user_id)]
    return web.json_response({"items": items})
...
async def create_or_set_budget(request: web.Request) -> web.Response:
    ...
    try:
        db.set_budget(user_id, category_id, limit)
    except PermissionError:
        return web.json_response({"error": "read-only"}, status=403)
    row = next(
        (b for b in db.get_budgets(user_id) if b["category_id"] == category_id),
        None,
    )
    if row is None:
        return web.json_response({"error": "update failed"}, status=400)
    return web.json_response(_serialize_budget(user_id, row), status=201)
...
async def patch_budget(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response({"error": "invalid id"}, status=400)
    row = db.get_budget_by_id(user_id, budget_id)
    if row is None:
        return web.json_response({"error": "not found"}, status=404)
    try:
        payload = await request.json()
    ...
    if limit is None:
        return web.json_response({"error": "invalid amount"}, status=400)
    try:
        db.set_budget(user_id, row["category_id"], limit)
    except PermissionError:
        return web.json_response({"error": "read-only"}, status=403)
    updated = db.get_budget_by_id(user_id, budget_id)
    return web.json_response(_serialize_budget(user_id, updated))
...
async def get_goals_status(request: web.Request) -> web.Response:
    ...
    if not user_id:
        return web.json_response({"error": "unauthorized"}, status=401)
    return web.json_response({"items": [_serialize_goal(g) for g in db.get_goals(user_id)]})
...
async def create_goal(request: web.Request) -> web.Response:
    ...
    if deadline == "":
        deadline = None
    try:
        goal_id = db.create_goal(user_id, name, target, deadline)
    except PermissionError:
        return web.json_response({"error": "read-only"}, status=403)
    except ValueError:
        return web.json_response({"error": "invalid deadline"}, status=400)
    row = db.get_goal_by_id(user_id, goal_id)
...
async def patch_goal(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response({"error": "invalid id"}, status=400)
    if db.get_goal_by_id(user_id, goal_id) is None:
        return web.json_response({"error": "not found"}, status=404)
    try:
        payload = await request.json()
    except (json.JSONDecodeError, TypeError):
    ...
    if not ok:
        return web.json_response({"error": "update failed"}, status=400)
    return web.json_response(_serialize_goal(db.get_goal_by_id(user_id, goal_id)))
...
async def contribute_goal(request: web.Request) -> web.Response:
    ...
    if amount is None:
        return web.json_response({"error": "invalid amount"}, status=400)
    try:
        tx_id = db.contribute_to_goal(user_id, goal_id, amount)
    except PermissionError:
        return web.json_response({"error": "read-only"}, status=403)
    if tx_id is None:
        return web.json_response({"error": "not found"}, status=404)
